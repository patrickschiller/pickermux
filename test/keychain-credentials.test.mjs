...
test("Keychain provider registry rejects hardlinks before parsing or mutation", async (t) => {
  const {
    root,
    registryDirectory,
    registryPath,
  } = await registryFixture(t, "hardlink-");
  await mkdir(registryDirectory, { mode: 0o700 });
  const foreignPath = path.join(root, "foreign-invalid-registry.json");
  const foreignContents = "not-json-and-must-remain-unchanged\n";
  await writeFile(foreignPath, foreignContents, { mode: 0o600 });
  await link(foreignPath, registryPath);
  ...
  await assert.rejects(
    registerKeychainProvider("vendor", { registryPath }),
    /exactly one hard link/iu,
  );
  await assert.rejects(
    purgeKeychainProviderRegistry({
  ...
  assert.equal(await readFile(registryPath, "utf8"), foreignContents);
  assert.equal((await stat(foreignPath)).nlink, 2);
  assert.equal((await stat(registryPath)).nlink, 2);
});
...
test("Keychain provider registry purge retains a byte-identical inode replacement", async (t) => {
  const { registryDirectory, registryPath } = await registryFixture(
    t,
    "purge-inode-drift-",
  );
  ...
  const result = await purgeKeychainProviderRegistry({
    ...
    beforeCommit: async () => {
      ...
      quarantinePath = path.join(registryDirectory, quarantineName);
      const raw = await readFile(quarantinePath);
      const replacement = `${quarantinePath}.replacement`;
      await writeFile(replacement, raw, { mode: 0o600 });
      await rename(replacement, quarantinePath);
    },
  });
  ...
  assert.equal(await exists(quarantinePath), true);
  assert.deepEqual(
    JSON.parse(await readFile(quarantinePath, "utf8")).providerIds,
    ["vendor"],
  );
});
