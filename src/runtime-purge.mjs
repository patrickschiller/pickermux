...
const RECEIPT_REMOVABLE_PARENT_ENTRIES = new Set([
  "bridge.log",
  "certifications.json",
  "compatibility.json",
  "models.json",
...
const BACKUP_QUARANTINE_PATTERN =
  /^\.backups\.purge\.[1-9]\d*\.[0-9a-f]{16}\.staging$/u;
const KEYCHAIN_REGISTRY_QUARANTINE_PATTERN =
...
async function inspectManagedParentEntries(serviceDirectory) {
  const parent = path.dirname(serviceDirectory);
  ...
  for (const name of names) {
    ...
    if (stats.isSymbolicLink()) {
      throw new Error(
        `Managed service package parent contains a symbolic link: ${name}`,
      );
    }
    assertOwned(stats, target);
    ...
    if (type === null) {
      throw new Error(
        `Managed service package parent contains an unsupported entry: ${name}`,
      );
    }
    entries.push(Object.freeze({ name, type, snapshot: snapshot(stats) }));
  }
  return Object.freeze(entries);
}

function sameParentEntry(left, right) {
  return (
...
function assertParentEntriesMatch(expected, current) {
  if (
    expected.length !== current.length ||
    expected.some((entry, index) => !sameParentEntry(entry, current[index]))
  ) {
    throw new Error("Managed service package parent entries changed during inventory");
  }
}

function findReceiptQuarantine(currentByName, pattern, expected) {
  const candidates = [...currentByName.values()].filter(
    (entry) => pattern.test(entry.name),
  );
  if (candidates.length !== 1) return null;
  const [candidate] = candidates;
  const identityMatches = expected.type === "directory"
    ? candidate.type === "directory" &&
      sameStableDirectoryIdentity(expected.snapshot, candidate.snapshot)
    : candidate.type === "file" &&
      expected.snapshot.nlink === 1 &&
      candidate.snapshot.nlink === 1 &&
      expected.snapshot.dev === candidate.snapshot.dev &&
      expected.snapshot.ino === candidate.snapshot.ino &&
      expected.snapshot.uid === candidate.snapshot.uid &&
      expected.snapshot.mode === candidate.snapshot.mode &&
      expected.snapshot.size === candidate.snapshot.size &&
      expected.snapshot.mtimeMs === candidate.snapshot.mtimeMs;
  return identityMatches ? candidate : null;
}

function assertReceiptBoundParentTransition(expected, current) {
  const currentByName = new Map(current.map((entry) => [entry.name, entry]));
  for (const before of expected) {
    const direct = currentByName.get(before.name);
    if (direct) {
      if (!sameParentEntry(before, direct)) {
        throw new Error(
          `Managed service package parent entry changed after inventory: ${before.name}`,
        );
      }
      currentByName.delete(before.name);
      continue;
    }
    if (RECEIPT_REMOVABLE_PARENT_ENTRIES.has(before.name)) continue;

    const quarantinePattern = before.name === "backups"
      ? BACKUP_QUARANTINE_PATTERN
      : before.name === "keychain-state.json"
        ? KEYCHAIN_REGISTRY_QUARANTINE_PATTERN
        : null;
    const quarantine = quarantinePattern
      ? findReceiptQuarantine(currentByName, quarantinePattern, before)
      : null;
    if (!quarantine) {
      throw new Error(
        `Managed service package parent entry disappeared after inventory: ${before.name}`,
      );
    }
    currentByName.delete(quarantine.name);
  }
  if (currentByName.size > 0) {
    throw new Error(
      `Managed service package parent gained entries after inventory: ${[
        ...currentByName.keys(),
      ].sort().join(", ")}`,
    );
  }
...
async function readRegularFile(target, initialStats) {
  if (
    ...
    throw new Error(
      `Service package file must be a regular file with one filesystem link: ${target}`,
    );
  }
...
export async function inventoryManagedServicePackage({
  ...
  const parentEntries = parentSnapshot === null
    ? Object.freeze([])
    : await inspectManagedParentEntries(roots.runtime);
  ...
  if (runtimeStats === null) {
    ...
    if (confirmedParent !== null) {
      assertParentEntriesMatch(
        parentEntries,
        await inspectManagedParentEntries(roots.runtime),
      );
    }
 ...
 */
async function revalidateManagedServicePackageInventoryWithPolicy(
  inventory,
  { allowReceiptBoundParentTransitions = false } = {},
) {
  assertIssuedInventory(inventory);
  if (typeof allowReceiptBoundParentTransitions !== "boolean") {
    throw new TypeError(
      "allowReceiptBoundParentTransitions must be a boolean",
    );
  }
  ...
  assertParentSnapshotMatches(inventory.parentSnapshot, parentBefore, {
    stableIdentityOnly: allowReceiptBoundParentTransitions,
  });
  ...
  const parentEntriesBefore = parentBefore === null
    ? Object.freeze([])
    : await inspectManagedParentEntries(inventory.serviceDirectory);
  if (allowReceiptBoundParentTransitions) {
    assertReceiptBoundParentTransition(
      inventory.parentEntries,
      parentEntriesBefore,
    );
  } else {
    assertParentEntriesMatch(inventory.parentEntries, parentEntriesBefore);
  }
  ...
  const parentEntriesAfter = parentAfter === null
    ? Object.freeze([])
    : await inspectManagedParentEntries(inventory.serviceDirectory);
  if (allowReceiptBoundParentTransitions) {
    assertReceiptBoundParentTransition(
    ...
    assertParentEntriesMatch(inventory.parentEntries, parentEntriesAfter);
  }
  return inventory;
}

/**
 * Revalidate one issued inventory against the exact parent and runtime state
 * captured during inventory. Lifecycle transitions are deliberately not part
 * of this general-purpose preflight API.
 */
export async function revalidateManagedServicePackageInventory(inventory) {
  return revalidateManagedServicePackageInventoryWithPolicy(inventory);
}
 ...
 * and empty directories. No recursive deletion is used. Parent transitions
 * remain strict by default; the lifecycle may explicitly admit only the
 * receipt-bound sibling removals and inode-preserving quarantines after its
 * strict preflight has completed.
 */
export async function removeInventoriedServicePackage({
  ...
  await revalidateManagedServicePackageInventoryWithPolicy(inventory, {
