...
function assertPrivateRegistryFile(stats, registryPath) {
  ...
  if (stats.nlink !== 1) {
    throw new Error(
      `Keychain provider registry must have exactly one hard link: ${registryPath}`,
    );
  }
...
function registryFileSnapshot(stats) {
  return Object.freeze({
    dev: stats.dev,
    ino: stats.ino,
    uid: stats.uid,
...
function sameStableRegistryFileIdentity(left, right) {
  return (
    left !== null &&
    right !== null &&
    left.dev === right.dev &&
...
async function readKeychainProviderRegistryState(
  ...
  try {
    ...
    fileSnapshot = registryFileSnapshot(confirmed);
  } finally {
    await handle.close();
  }
...
async function assertRegistryUnchanged(registryPath, previous) {
  ...
  if (
    ...
    (current.raw !== null && (
      !current.raw.equals(previous.raw) ||
      current.sha256 !== previous.sha256 ||
      !sameStableRegistryFileIdentity(current.snapshot, previous.snapshot)
    ))
  ) {
...
function sameRegistryState(left, right, expectedProviderIds) {
  return (
    ...
    sameStableRegistryFileIdentity(left.snapshot, right.snapshot) &&
